...
# def analyze_proposal_components(proposal_text):
...
#             ai_analysis = gemini.analysis_proposal(proposal_text)
...
#         if "communication plan" in analysis_lower and "true" in analysis_lower:
#             components["Communication Plan"] = True
...
#         components["Success Criteria"] = True
#     if any(keyword in text_lower for keyword in ["communication", "reporting", "meetings", "updates"]):
#         components["Communication Plan"] = True
...
#     st.session_state.mode = "with_proposal"  # Default to with_proposal
# if 'step' not in st.session_state:
#     st.session_state.step = 1
# if 'rfp_text' not in st.session_state:
#     st.session_state.rfp_text = ""
...
#                 st.write("Please upload your proposal document in PDF format for analysis.")
...
#                     components, ai_details = analyze_proposal_components(st.session_state.proposal_text)
...
#                             st.markdown(f'<div class="{card_class}"><strong>{icon} {component}</strong></div>', unsafe_allow_html=True)
...
#                             st.session_state.ai_analysis_details or str(st.session_state.proposal_analysis)
#                         )
...
#                     #         pdf.cell(200, 10, txt="Executive Summary: RFP Evaluation", ln=1, align='C')
#                     #         pdf.ln(10)
                            
#                     #         # Add content with proper formatting
#                     #         pdf.set_font("Arial", size=12)
#                     #         for line in st.session_state.proposal_summary.split('\n'):
...
#                     #                 pdf.cell(200, 10, txt=line.lstrip('#').strip(), ln=1)
#                     #                 pdf.set_font("Arial", size=12)
#                     #             else:
...
#                     #     # For displaying the content in the app
...
#                     # Back button
#                     if st.button("⬅️ Back to Component Analysis", use_container_width=True):
#                         st.session_state.step = 2
#                         st.rerun()
#     with col2:
#         lottie_url = "https://assets5.lottiefiles.com/packages/lf20_bhebjzpu.json"
#         lottie_json = load_lottie_url(lottie_url)
#         if lottie_json:
#                 st_lottie(lottie_json, height=200, key="document")

#         else:
#             st.warning("Please provide Proposal pdf.")
...
if st.session_state.mode == "with_proposal":
    ...
    # Fixed right column - Image stays in place
    with col2:
        # Container to keep the image fixed
        with st.container():
            lottie_url = "https://assets5.lottiefiles.com/packages/lf20_bhebjzpu.json"
            lottie_json = load_lottie_url(lottie_url)
            if lottie_json:
                st_lottie(lottie_json, height=200, key="document_fixed")
